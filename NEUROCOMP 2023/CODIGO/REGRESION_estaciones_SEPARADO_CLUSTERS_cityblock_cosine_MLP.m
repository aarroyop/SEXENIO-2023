...
%meses=12;
linea=2;
for (z=1:4) %4 ESTACIONES
%       clear all;
%         close all;
    for (w= 1:2) %citylock y cosine
        for (y=1:3) %3 clusters
      close all;
      clearvars -except z w y xx zz yy linea
      if (z==1) zz='inv'; end;
      if (z==2) zz='pri'; end;
      if (z==3) zz='ver'; end;
      if (z==4) zz='oto'; end;
      if (w==1) ww='city'; end;
      if (w==2) ww='cosine'; end;
      if (y==1) yy='k1'; end;
      if (y==2) yy='k2'; end;
      if (y==3) yy='k3'; end;
      %zz=int2str(z); ww =int2str(w); 
      entrada=strcat('kmeans_',ww,'_',yy,'_',zz,'_MLP.txt');    
...
%writematrix(prueba,'enerosalida.txt','Delimiter',';') %para escribir la
%matriz en un fichero de texto
...
% if (mes>1) B9={mes,tocPLMtrainSTD,tocPGDXtrainSTD,tocPBtrainSTD,tocPSCGtrainSTD,tocPBRtrainSTD}; end;
 if (z==1 && w==1 && y==1) C = { 'Season','Distance','Cluster','Time R Lineal','Std Time R Lineal','MSE Mean','STD MSE';zz,ww,yy,tocRLinealmed,tocRLinealstd,MedRLineal,ssmeRLineal}; end;
 if (z>1 || w>1 || y>1) C={zz,ww,yy,tocRLinealmed,tocRLinealstd,MedRLineal,ssmeRLineal}; end;

if (z==1 && w==1 && y==1) D = {'Season','Distance','Cluster','Time R Lineal','Std Time R Lineal','MSE Mean','STD MSE';zz,ww,yy,tocELMmed,tocELMstd,MedELM,ssmeELM}; end;
if (z>1 || w>1 || y>1) D={zz,ww,yy,tocELMmed,tocELMstd,MedELM,ssmeELM}; end;

% if (z==1 && w==1 && y==1) E = {'Season','Distance','Cluster','Time R Lineal','Std Time R Lineal','MSE Mean','STD MSE';zz,ww,yy,tocSVMmed,tocSVMstd,MedSVM,ssmeSVM}; end;
% if (z>1 || w>1 || y>1) E={zz,ww,yy,tocSVMmed,tocSVMstd,MedSVM,ssmeSVM}; end;
...
%  if (z>1 || w>1) xlswrite(filename,C,'Regression LINEAL',f); end;
  ...
  if (z==1 && w==1 && y==1) xlswrite(filename,C,'Reg. LM 4 Seasons Clustering','A1'); end;
 if (z>1 || w>1 || y>1) xlswrite(filename,C,'Reg. LM 4 Seasons Clustering',f); end;
 if (z==1 && w==1 && y==1) xlswrite(filename,D,'Reg. ELM 4 Seasons Clustering','A1'); end;
  if (z>1 || w>1 || y>1) xlswrite(filename,D,'Reg. ELM 4 Seasons Clustering',f); end;
 linea=linea+1;
