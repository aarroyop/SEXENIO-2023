...
%         close all;
    for (y= 1:2)
        ...
        entrada=strcat('Kmeans_',medida,'_k',zz,'_MLP.txt');    
...
if (y==1 && z==1) C = { 'Measure','Cluster','Time R Lineal','Std Time R Lineal','MSE Mean','STD MSE';y,z,tocRLinealmed,tocRLinealstd,MedRLineal,ssmeRLineal}; end;
 if ( y>1 || z>1) C={y,z,tocRLinealmed,tocRLinealstd,MedRLineal,ssmeRLineal}; end;

if ( y==1 && z==1) D = {'Measure','Cluster','Time R Lineal','Std Time R Lineal','MSE Mean','STD MSE';y,z,tocELMmed,tocELMstd,MedELM,ssmeELM}; end;
if ( y>1 || z>1) D={y,z,tocELMmed,tocELMstd,MedELM,ssmeELM}; end;
...
f=int2str(linea); 
f=strcat('A',f);
 if ( y==1 && z==1) xlswrite(filename,D,'Reg. ELM 1 YEAR Clus','A1'); end;
 if (y>1 || z>1) xlswrite(filename,D,'Reg. ELM 1 YEAR Clus',f); end;
  if ( y==1 && z==1) xlswrite(filename,C,'Reg. LM 1 YEAR Clus','A1'); end;
  if ( y>1 || z>1) xlswrite(filename,C,'Reg. LM 1 YEAR Clus',f); end;
  linea=linea+1;
