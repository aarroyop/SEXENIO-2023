...
for z=1:12
  ...
  if (z==1) datos=3310; entrada='enero.txt'; end;
  if (z==2) datos=3307; entrada='febrero.txt'; end;
   if (z==3) datos=3345; entrada='marzo.txt';  end;
  if (z==4) datos=2728; entrada='abril.txt';  end;
  if (z==5) datos=3018; entrada='mayo.txt'; end;
  if (z==6) datos=3050; entrada='junio.txt';  end;
  if (z==7) datos=3000; entrada='julio.txt'; end;
  if (z==8) datos=2999; entrada='agosto.txt';  end;
  if (z==9) datos=2828; entrada='septiembre.txt'; end;
  if (z==10) datos=2094; entrada='octubre.txt';  end;
  if (z==11) datos=1963; entrada='noviembre.txt'; end;
  if (z==12) datos=2994; entrada='diciembre.txt'; end;
fd = fopen(entrada,'r');
filename='resultadoglobal4.xlsx';
mes=z;
if (fd<0)
    'fopen failed'
    return;
...
if (mes==1) C = {'month','Time R Lineal','Std Time R NO Lineal','MSE Mean','MSE STD';mes,tocRLinealmed,tocRLinealstd,MedRLineal,ssmeRLineal}; end;
if (mes>1) C={mes,tocRLinealmed,tocRLinealstd,MedRLineal,ssmeRLineal}; end;
...
if (mes==1) xlswrite(filename,C,'Regression LINEAL','A1'); end;
f=mes+1; f=int2str(f); f=strcat('A',f);
if (mes>1) xlswrite(filename,C,'Regression LINEAL',f); end;
